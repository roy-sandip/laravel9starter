                    ...
                    <div class="row">
                       <div class="col-12">
                           <table class="table table-sm" style="text-align:right;font-size: 18px;">
                            ...
                            <tbody>
                               ...
                               <tr>
                                   <td colspan="6">Last invoice ({{$invoice->lastInvoice->id}})</td>
                                   <td colspan="2">{{$invoice->lastInvoice->balance}}</td>
                                   <td colspan="2"></td>
                               </tr>
                               @endif
                           </tbody>
                       ...
                       <div class="row">
                           <div class="col-12">
                                ...
                                <form action="{{route('admin.invoices.transaction.update', $item->id)}}" method="post" class="transactionForm">
                                    @method('put')
                                <table class="table table-sm">
                                   
                                <tr>
                                    <td>
                                        <button class="btn btn-sm btn-outline-danger deleteTXN" type="button" data-action="{{route('admin.invoices.transaction.delete', $item->id)}}"><i class="fas fa-times"></i></button>
                                    </td>
                                    <td>
                                        <input name="comment" type="text" class="form-control" max="190" value="{{$item['comment']}}" placeholder="Description">
                                    </td>
                                    <td>
                                        <div class="input-group"><div class="input-group-prepend"><div class="input-group-text"><i class="fas fa-calendar-alt"></i></div></div><input name="datetime" type="text" value="{{$item['datetime']}}" class="form-control datepicker" max="190" placeholder="Date"></div>
                                    </td>
                                    <td>
                                        <select name="type" id=""  onchange="calculateTXN(this)" class="form-control type">
                                            <option value="credit" @if(substr($item['type'], 0, 1) == 'c') selected @endif >+</option>
                                            <option value="debit"  @if(substr($item['type'],0,1) == 'd') selected @endif >-</option>
                                        </select>
                                    </td>
                                    <td>
                                        <div class="input-group">
                                            ...
                                            <input name="amount" type="number" value="{{$item['amount']}}" data-amount="{{$item['amount']}}" class="form-control text-right numeric amount " placeholder="Amount" min="0" max="99999999"  onchange="calculateTXN(this)" required=""> 
                                            <div class="input-group-append submitBtn" style="display: none;">
                            ...
                            <form action="{{route('admin.invoices.update', $invoice->id)}}" method="post" id="invoiceForm" onkeydown="return event.key != 'Enter';" >
                                @csrf
                                @method('put')
                               ...
                               <table class="table table-sm" id="transactions">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input name="txn[{{$key}}][comment]" type="text" class="form-control" max="190" value="{{$item['comment']}}" placeholder="Description">
                                    </td>
                                    <td>
                                        <div class="input-group"><div class="input-group-prepend"><div class="input-group-text"><i class="fas fa-calendar-alt"></i></div></div><input name="txn[{{$key}}][datetime]" type="text" value="{{$item['datetime']}}" class="form-control datepicker" max="190" placeholder="Date"></div>
                                    </td>
                                    <td>
                       ...
                       <div class="row">
                           <div class="col">
                            <button class="btn btn-outline-primary text-right" style="float: left;" onclick="addRow(rowid)" type="button">Add Transaction</button>
                               <button class="btn btn-success" type="submit" form="invoiceForm" style="float: right;">Update</button>
                           </div>
                       </div>
...
<script>
    ...
    function addRow(id)
    {
        var row = '<tr>';
            ...
            row += '<td><input name="txn['+id+'][comment]" type="text" class="form-control" max="190" placeholder="Description"/></td>';
            row += '<td><div class="input-group"><div class="input-group-prepend"><div class="input-group-text"><i class="fas fa-calendar-alt"></i></div></div><input name="txn['+id+'][datetime]" type="text" class="form-control datepicker" max="190" value="{{date('d F, Y')}}" placeholder="Date"/></div></td>';
...
$(".deleteTXN").on('click', function(event){
    event.preventDefault();
    ...
    $.ajax({
        ...
        beforeSend: function(){
            button.attr('disabled', true).find('i').removeClass('fa-times').addClass('fa-spin fa-spinner');
        },
        success: function(response){
            laravelSuccess(response, 'toast');
            $("#subTotal").html(response.balance);
        ...
        error: function(error){
            button.attr('disabled', false).find('i').addClass('fa-times').removeClass('fa-spin fa-spinner');
            laravelError(error);
        }
    });
});
...
var timeout;
$('.transactionForm :input').on('change input', function() {
        $(this).closest('tr').find('.submitBtn').show();
});

$('.transactionForm').on('submit', function(event){
    event.preventDefault();
    var button = $(this).find('.submitBtn button');
    var form = this;
    makeAjax(
        form,
        function(){
